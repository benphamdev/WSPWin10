...
//		cauSo10();
	...
	static void cauSo1(String maNhomNV) {
		...
		if (maTruongNhom.isPresent()) {
			...
			System.out.println(
					"Họ tên : " + x.getfullName() + " có địa chỉ : " + x.getAddress());
		} else {
...
//	Câu hỏi : Cho biết những loại phòng được khách thuê nhiều nhất
	static void cauSo() {
		List<Room> mergedRooms = roomList.stream().map(room -> {
			boolean isRoomUsed = bookingList.stream()
					.anyMatch(booking -> booking.getRoomId().equals(room.getroomId()));
			room.setStatus(isRoomUsed ? "het" : "con");
			return room;
		}).collect(Collectors.toList());

		Map<String, List<Room>> roomsByType = mergedRooms.stream()
				.collect(Collectors.groupingBy(Room::getRoomTypeId));

		Map<String, Long> roomUsageCountByType = roomsByType.entrySet().stream()
				.collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().stream()
						.filter(room -> room.getStatus().equals("het")).count()));

		Optional<Map.Entry<String, Long>> mostUsedRoomType = roomUsageCountByType
				.entrySet().stream().max(Comparator.comparing(Map.Entry::getValue));

		mostUsedRoomType
				.ifPresent(e -> System.out.println("Loại phòng được thuê nhiều nhất là "
						+ e.getKey() + " với số lượng thuế là " + e.getValue()));
	}
